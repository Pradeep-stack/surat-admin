...
import { updateApplicationAsync } from "../features/application/applicationThunk";
import Loader from "./Loader";
import { getHours } from "../api/Hours";

const UpdateDeclarationForm = () => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  useEffect(() => {
    if (applicationData) {
      setFormData({
        twoYearOld: applicationData.twoYearOld,
        sixDigitCodeCYC: applicationData.sixDigitCodeCYC,
        twoYearOldWorkParent: applicationData.twoYearOldWorkParent,
        elevenDigitCodeCYC: applicationData.elevenDigitCodeCYC,
        threeAndFourUH: applicationData.threeAndFourUH,
        threeAndFourEH: applicationData.threeAndFourEH,
        elevenDigitCodeCC: applicationData.elevenDigitCodeCC,
        entitlementType: applicationData.entitlementType,
        forename: applicationData.forename,
        surname: applicationData.surname,
        dob: applicationData.dob,
        address: applicationData.address,
        ethnicity: applicationData.ethnicity,
        gender: applicationData.gender,
        confirmDOB: applicationData.confirmDOB,
        birthCertificate: applicationData.birthCertificate,
        providerSignature: applicationData.providerSignature,
        dateRecorded: applicationData.dateRecorded,
        parentsForename: applicationData.parentsForename,
        parentsSurname: applicationData.parentsSurname,
        nationalInsuranceNumber: applicationData.nationalInsuranceNumber,
        parentsDateofbirth: applicationData.parentsDateofbirth,
        termlyFundedHours: applicationData.termlyFundedHours,
        applyForEYPP: applicationData.applyForEYPP,
        daf: applicationData.daf,
        agreeOne: applicationData.agreeOne,
        agreeTwo: applicationData.agreeTwo,
        agreeThree: applicationData.agreeThree,
        agreeFour: applicationData.agreeFour,
        agreeFive: applicationData.agreeFive,
        agreeSix: applicationData.agreeSix,
        providerA: applicationData.providerA,
        providerB: applicationData.providerB,
        providerC: applicationData.providerC,
        appropriateFundingTerm: applicationData.appropriateFundingTerm,
        parentSDate: applicationData.parentSDate,
      });
    }
  }, [applicationData]);
  ...
  //   if (imgData.providerSignature) {
  //     const providerFormData = new FormData();
  //     providerFormData.append("image", imgData.providerSignature);
  //     const providerResponse = await uploadImage(providerFormData);
  //     if (providerResponse) {
  //       setFormData((prevData) => ({
  //         ...prevData,
  //         providerSignature: providerResponse.data.filePath,
  //       }));
  //     }
  //   }

  //   if (imgData.parentSignature) {
  //     const parentFormData = new FormData();
  //     parentFormData.append("image", imgData.parentSignature);
  //     const parentResponse = await uploadImage(parentFormData);
  //     if (parentResponse) {
  //       setFormData((prevData) => ({
  //         ...prevData,
  //         parentSignature: parentResponse.data.filePath,
  //       }));
  //     }
  //   }
  // };
